...
export class Lexer {
	...
	constructor(document: TextDocument, scriptdir?: string) {
		...
		this.beautify = function (options?: FormatOptions) {
			let i: number, keep_whitespace: boolean | undefined, sweet_code: string;
			if (!_this.isparsed)
				_this.parseScript();
			lst = EMPTY_TOKEN;
			opt = Object.assign({
			...
			for (let i = 0, c = input.charAt(0); c === ' ' || c === '\t'; c = input.charAt(++i))
				preindent_string += c;
			is_conditional = false, begin_line = format_mode = true, parser_pos = 0, last_LF = -1;
			while (true) {
		...
		function format_params_default_val(tokens: { [offset: number]: Token }, params: Variable[]) {
			opt = Object.assign({}, FORMAT_DEFAULT_OPTS);
			space_in_other = Boolean(opt.space_in_other);
			indent_string = opt.indent_string ?? '\t';
					...
					switch (tk.type) {
						...
						case 'TK_HOTLINE': {
							...
							if (k.match(allIdentifierChar) && !k.match(/^[$\d]/) && !k.match(KEYS_RE)) {
								...
								delete tk.data;
								if (lk.type === 'TK_WORD') {
									let ci = DocumentSymbol.create(lk.content, undefined,
										SymbolKind.Function, make_range(lk.offset, parser_pos - 1),
										make_range(lk.offset, lk.length)) as CallInfo;
									ci.offset = lk.offset, ci.paraminfo = { count: 0, miss: [], comma: [], unknown: false };
									addvariable(lk, 0, tn.children = []);
									(<FuncNode>tn).funccall = [ci];
								} else
							...
							if (tk.content === '[') {
								...
								if (lk.type === 'TK_WORD' && nospace)
									if (input.charAt(lk.offset - 1) === '.') {
										...
										} else ptk.semantic = { type: SemanticTokenTypes.variable };
										continue;
									} else fc = lk;
								...
								if (_this.get_token(parser_pos).content === '=>') {
									...
									if (fc) {
										...
										} else fc.semantic = { type: SemanticTokenTypes.variable };
									} else {
							...
							if (input.charAt(parser_pos) !== '(') {
								...
								if (tk.content === '=>') {
									...
									if (input.charAt(fc.offset - 1) !== '%') {
										...
										fc.semantic = { type: SemanticTokenTypes.variable };
									next = false;
		...
		function handle_word() {
			...
			if (start_of_statement()) {
				...
				(opt.preserve_newlines || !(last_type === 'TK_RESERVED' && ['local', 'static', 'global'].includes(flags.last_text)))) {
				print_newline();
				not_add_line = false;
			}
