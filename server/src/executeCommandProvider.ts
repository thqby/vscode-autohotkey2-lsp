...
async function generateComment(args: string[]) {
	...
	let doc = lexers[uri = uri.toLowerCase()], scope = doc.searchScopedNode(pos);
	if (scope) {
		if (scope.kind === SymbolKind.Function || scope.kind === SymbolKind.Method) {
...
async function generateAuthor() {
	...
	if (tk.type.endsWith('COMMENT')) {
		if (tk.type === 'TK_BLOCK_COMMENT' && tk.content.indexOf('@') !== -1)
			return;
	}
